"""Tests for `evaluate_watch` — Hook 4 user contribution.

The default stub only fires TARGET_HIT with a "must beat last alerted price"
debounce. These tests pass against the default. As you add richer policy
(PRICE_DROP without a target, NEW_LOW, percentage debounce), the marked
tests turn green.
...
@pytest.mark.xfail(reason="Implement PRICE_DROP for targetless watches", strict=False)
...
def test_small_jitter_without_target_does_not_fire():
    ...
    Passes trivially today (no target → never fires). Once you implement
    PRICE_DROP, this becomes the guard that keeps your threshold honest.
    """
    d = evaluate_watch(obs("784"), snap(last="800", lowest="780"))
    assert not d.fire


@pytest.mark.xfail(reason="Implement NEW_LOW", strict=False)
...
@pytest.mark.xfail(reason="Implement percentage-based re-alert debounce", strict=False)
